...
def upgrade():
    ...
    role_permissions = {
        ...
        "creator": [
            "view_users", "view_content", "generate_content", "summarize_content",
            "view_notifications", "start_chat", "view_chat", "send_message"
        ],
    ...
    for role, perms in role_permissions.items():
        role_id = conn.execute(
            sa.text("SELECT id FROM roles WHERE name=:role"),
            {"role": role}
        ).scalar()
...
def downgrade():
    ...
    conn = op.get_bind()
    perm_names = [
        "view_users", "edit_users", "delete_users", "view_content", "edit_content", "delete_content",
        "generate_content", "summarize_content", "view_notifications", "send_notifications",
        "start_chat", "view_chat", "send_message", "view_usage"
    ]
    for name in perm_names:
        perm_id = conn.execute(sa.text("SELECT id FROM permissions WHERE name=:name"), {"name": name}).scalar()
        if perm_id:
            conn.execute(sa.text("DELETE FROM role_permissions WHERE permission_id=:perm_id"), {"perm_id": perm_id})
            conn.execute(sa.text("DELETE FROM permissions WHERE id=:perm_id"), {"perm_id": perm_id})
