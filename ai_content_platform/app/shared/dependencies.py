...
async def get_user_permissions(current_user: User = Depends(get_current_user)):
	logger.info(f"Fetching permissions for user: {getattr(current_user, 'username', None)}")
	try:
		permissions = set()
	...
	async def role_dependency(user: User = Depends(get_current_user)):
		try:
			if getattr(user, "role", None) != required_role:
				logger.warning(f"Role '{required_role}' required, but user has role '{getattr(user, 'role', None)}'")
				raise HTTPException(
					status_code=status.HTTP_403_FORBIDDEN,
					detail=f"Insufficient privileges: requires {required_role}",
				)
			logger.info(f"Role '{required_role}' granted for user: {getattr(user, 'username', None)}")
			return user
		except HTTPException:
			raise
		except Exception as e:
