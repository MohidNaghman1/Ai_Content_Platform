...
from ai_content_platform.app.modules.notifications.schemas import NotificationCreate, NotificationResponse, MarkReadRequest
...
def send_notification(
    ...
    """
    logger.info(f"API: Sending notification to user {notification.user_id} of type {notification.type}")
    try:
        publish_event(
...
def get_user_notifications(
    ...
    """
    logger.info(f"API: Fetching notifications for user {user_id}, unread_only={unread_only}, limit={limit}")
    try:
        service = NotificationService(db=db)
        notifications = service.get_user_notifications(user_id, unread_only, limit)
        return notifications
    except Exception as e:
        logger.error(f"API: Error fetching notifications for user {user_id}: {e}", exc_info=True)
...
def get_unread_count(
    ...
    try:
        ...
        logger.error(f"API: Error fetching unread count for user {user_id}: {e}", exc_info=True)
...
def mark_notification_as_read(
    ...
    """
    logger.info(f"API: Marking notification {notification_id} as read for user {request.user_id}")
    try:
        service = NotificationService(db=db)
        success = service.mark_as_read(notification_id, request.user_id)
        if not success:
            logger.warning(f"API: Notification {notification_id} not found for user {request.user_id}")
            raise HTTPException(
                status_code=404, 
        ...
        logger.error(f"API: Error marking notification {notification_id} as read: {e}", exc_info=True)
...
def mark_all_notifications_as_read(
    ...
    try:
        ...
        logger.error(f"API: Error marking all notifications as read for user {user_id}: {e}", exc_info=True)
...
def delete_notification(
    ...
    try:
        ...
        if not success:
            logger.warning(f"API: Notification {notification_id} not found for user {user_id}")
            raise HTTPException(
                status_code=404,
        ...
        logger.error(f"API: Error deleting notification {notification_id} for user {user_id}: {e}", exc_info=True)
...
def get_user_notification_preferences(
    ...
    try:
        ...
        logger.error(f"API: Error fetching notification preferences for user {user_id}: {e}", exc_info=True)
