...
class NotificationService:
    ...
    def get_user_preferences(self, user_id: int) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.error(
                f"Error fetching user preferences for user {user_id}: {e}",
                exc_info=True,
            )
            return {"email_notifications": True, "in_app_notifications": True}
    ...
    def process_notification_event(self, event: dict) -> Dict:
        ...
        try:
            ...
            if not user_id or not message:
                logger.error(
                    "[Notification] Invalid event data: missing user_id or message"
                )
            ...
            if event_type == "email":
                if preferences.get("email_notifications"):
                    ...
                    logger.info(
                        f"[Notification] Skipped email for user {user_id} due to preferences"
                    )
                ...
                if preferences.get("in_app_notifications"):
                    ...
                    logger.info(
                        f"[Notification] Skipped in_app for user {user_id} due to preferences"
                    )
            ...
            logger.info(
                f"[NotificationService] Processed event for user {user_id}: {results}"
            )
            return results
        except Exception as e:
    ...
    def send_email_notification(
        self, user_id: int, message: str, to_email: Optional[str] = None
    ) -> Notification:
        """
        ...
        try:
            ...
            logger.info(
                f"[EMAIL] To: {to_email} | User ID: {user_id} | Message: {message}"
            )
            # Log email in database
            email_log = Notification(
                user_id=user_id, message=message, notif_type="email", read=False
            )
            if self.db:
            ...
            logger.error(
                f"Error sending email notification to user {user_id}: {e}",
                exc_info=True,
            )
            raise

    def send_in_app_notification(self, user_id: int, message: str) -> Notification:
        """
        ...
        try:
            ...
            notif = Notification(
                user_id=user_id, message=message, notif_type="in_app", read=False
            )
            # Save to database
            if self.db:
                self.db.add(notif)
            ...
            logger.info(
                f"[In-App] Notification created with ID: {notif.id if notif.id else 'N/A'}"
            )
            return notif
        except Exception as e:
            logger.error(
                f"Error sending in-app notification to user {user_id}: {e}",
                exc_info=True,
            )
            raise
    ...
    def get_user_notifications(
        ...
        """
        logger.info(
            f"Fetching notifications for user {user_id}, unread_only={unread_only}, limit={limit}"
        )
        try:
            ...
            query = self.db.query(Notification).filter(
                Notification.user_id == user_id, Notification.notif_type == "in_app"
            )
            if unread_only:
            ...
            logger.error(
                f"Error fetching notifications for user {user_id}: {e}", exc_info=True
            )
            return []
    ...
    def mark_as_read(self, notification_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error marking notification {notification_id} as read for user {user_id}: {e}",
                exc_info=True,
            )
            return False
    ...
    def delete_notification(self, notification_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error deleting notification {notification_id} for user {user_id}: {e}",
                exc_info=True,
            )
            return False
    ...
    def get_unread_count(self, user_id: int) -> int:
        ...
        try:
            ...
            logger.error(
                f"Error getting unread count for user {user_id}: {e}", exc_info=True
            )
            return 0
