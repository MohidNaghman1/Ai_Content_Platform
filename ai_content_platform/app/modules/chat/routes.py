...
from ai_content_platform.app.shared.dependencies import get_db, get_current_user, require_permission
from ai_content_platform.app.modules.chat.schemas import ConversationCreate, ConversationOut, MessageCreate, MessageOut, TokenUsageOut
...
async def get_conversation(conversation_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    logger.info(f"Get conversation endpoint called for user: {user['sub']}, conversation_id: {conversation_id}")
    try:
        conv = await services.get_conversation(db, conversation_id)
        if not conv or conv.user_id != user["sub"]:
            logger.warning(f"Conversation not found: {conversation_id} for user: {user['sub']}")
...
# Improved streaming logic: StreamingResponse is returned immediately, DB save is handled after streaming
@chat_router.post(
...
async def stream_message(
    ...
    logger.info(f"Stream message endpoint called for user: {user['sub']}, conversation_id: {conversation_id}")
    try:
        conv = await services.get_conversation(db, conversation_id)
        if not conv or conv.user_id != user["sub"]:
            logger.warning(f"Conversation not found: {conversation_id} for user: {user['sub']}")
            raise HTTPException(404, "Conversation not found")
        # Add user message
        user_msg = await services.add_message(db, conversation_id, sender="user", content=msg.content)
        # Parse keywords
        keywords = [k.strip() for k in retrieval_keywords.split(",") if k.strip()] if retrieval_keywords else []
            ...
            try:
                async for chunk in services.stream_ai_response(
                    ...
                    if len(full_response) + len(chunk) > MAX_RESPONSE_CHARS:
                        ...
                        full_response += chunk[:MAX_RESPONSE_CHARS - len(full_response)]
                        incomplete = True
                        break
                    full_response += chunk
                    yield chunk
        ...
        async def save_ai_response_and_summary_bg():
            ...
            async for db_bg in get_db():
                try:
                    ...
                    await services.add_message(db_bg, conversation_id, sender="assistant", content=meta_content)
                    await services.track_token_usage(db_bg, conversation_id, user["sub"], tokens=len(msg.content) + len(ai_stream_accum.full_response))
                    # Only summarize if response is complete
                    if not ai_stream_accum.incomplete:
                        await services.update_conversation_summary(db_bg, conversation_id)
                except Exception as e:
                    logger.error(f"Error in save_ai_response_and_summary_bg: {e}", exc_info=True)
                finally:
                    await db_bg.close()
                break
        ...
        return StreamingResponse(ai_stream_accum(), media_type="text/plain", background=background_tasks)
    except Exception as e:
        logger.error(f"Error in stream_message: {e}", exc_info=True)
        raise
async def get_messages(conversation_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    logger.info(f"Get messages endpoint called for user: {user['sub']}, conversation_id: {conversation_id}")
    try:
        conv = await services.get_conversation(db, conversation_id)
        if not conv or conv.user_id != user["sub"]:
            logger.warning(f"Conversation not found: {conversation_id} for user: {user['sub']}")
...
async def get_token_usage(conversation_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    logger.info(f"Get token usage endpoint called for user: {user['sub']}, conversation_id: {conversation_id}")
    try:
        conv = await services.get_conversation(db, conversation_id)
        if not conv or conv.user_id != user["sub"]:
            logger.warning(f"Conversation not found: {conversation_id} for user: {user['sub']}")
        ...
        logger.info(f"Token usage records fetched for conversation_id: {conversation_id}")
        return [TokenUsageOut(tokens_used=u.tokens_used, created_at=u.created_at) for u in usage_records]
    except Exception as e:
