...
from ai_content_platform.app.modules.users.services import get_user_by_username,create_user
from ai_content_platform.app.modules.auth.services import (
    authenticate_user, issue_tokens, rotate_refresh_token, revoke_refresh_token
)
...
async def refresh_token(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        from ai_content_platform.app.modules.users.models import User
        new_access_token, new_refresh_token, user = await rotate_refresh_token(refresh_token, db, User)
        if not new_access_token:
            logger.warning("Invalid refresh token provided")
            raise HTTPException(401, "Invalid refresh token")
        logger.info(f"Refresh token successful for user: {getattr(user, 'username', None)}")
        return {
