# Admin dashboard routes for user management, content moderation, analytics, and system health monitoring
...
from ai_content_platform.app.modules.content.schemas import ArticleOut, ArticleCreate, ArticleUpdate
...
@admin_router.get("/moderation/flagged", response_model=list[ArticleOut], dependencies=[Depends(require_role("admin"))])
...
@admin_router.post("/users", response_model=UserOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_role("admin"))])
...
@admin_router.delete("/users/{user_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role("admin"))])
async def admin_delete_user(user_id: int):
    return await delete_user_service(user_id)

# Article management endpoints
@admin_router.get("/articles", response_model=list[ArticleOut], dependencies=[Depends(require_role("admin"))])
async def admin_list_articles():
    return await get_all_articles()

@admin_router.post("/articles", response_model=ArticleOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_role("admin"))])
async def admin_create_article(article: ArticleCreate):
    return await create_article_service(article)

@admin_router.put("/articles/{article_id}", response_model=ArticleOut, dependencies=[Depends(require_role("admin"))])
async def admin_update_article(article_id: int, update: ArticleUpdate):
    return await update_article_service(article_id, update)

@admin_router.delete("/articles/{article_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_role("admin"))])
...
@admin_router.post("/moderation/{article_id}/review", response_model=ArticleOut, dependencies=[Depends(require_role("admin"))])
