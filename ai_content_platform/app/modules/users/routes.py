...
from ai_content_platform.app.shared.dependencies import get_current_user, get_db, require_permission
from ai_content_platform.app.modules.users.schemas import UserCreate, UserOut, UserUpdate
from ai_content_platform.app.modules.users.services import create_user, get_user_by_username, get_password_hash
...
@user_router.get("/", response_model=list[UserOut], dependencies=[Depends(require_permission("view_users"))])
...
async def get_profile(
    ...
    try:
        ...
        logger.error(f"API: Error fetching profile for user {current_user.username}: {e}", exc_info=True)
        raise HTTPException(500, "Failed to fetch user profile")

# Update current user's profile
@user_router.put("/me", response_model=UserOut)
async def update_profile(update: UserUpdate, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error(f"API: Error updating profile for user {current_user.username}: {e}", exc_info=True)
...
async def create_user_endpoint(user_in: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        user = await create_user(db, user_in.username, user_in.email, user_in.password, user_in.role)
        return user
    except Exception as e:
...
async def upload_avatar(avatar: str, current_user=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Update the current user's avatar (string path)."""
    logger.info(f"API: Uploading avatar for user {current_user.username}")
    try:
        user = await get_user_by_username(db, current_user.username)
        if not user:
            logger.warning(f"API: User not found for avatar upload: {current_user.username}")
            raise HTTPException(status_code=404, detail="User not found")
        user.avatar = avatar
        await db.commit()
        await db.refresh(user)
        ...
        logger.error(f"API: Error uploading avatar for user {current_user.username}: {e}", exc_info=True)
