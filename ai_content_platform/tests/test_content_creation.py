# Ensure user is assigned admin role in DB (in case registration does not
# do it)
...
def mock_gemini(monkeypatch):
    ...
    monkeypatch.setattr(
        gemini_service.GeminiService, "generate_text", mock_generate_text
    )
...
async def test_create_article_with_new_and_existing_tags(client):
    ...
    async def ensure_admin_and_permission():
        ...
        summ_perm = (
            await session.execute(
                select(Permission).where(Permission.name == "summarize_content")
            )
        ).scalar_one_or_none()
        if not summ_perm:
            summ_perm = Permission(
                name="summarize_content", description="Can summarize articles"
            )
        ...
        summ_rp_exists = (
            await session.execute(
                role_permissions.select().where(
                    ...
                    & (role_permissions.c.permission_id == summ_perm.id)
                )
            )
        ).first()
        if not summ_rp_exists:
            await session.execute(
                role_permissions.insert().values(
                    role_id=admin_role.id, permission_id=summ_perm.id
                )
            )
            await session.commit()
    ...
    async with AsyncTestingSessionLocal() as session:
        ...
        user = (
            await session.execute(
                select(users_table).where(users_table.c.username == "alice_test")
            )
        ).first()
        ...
        perm = (
            await session.execute(
                select(Permission).where(Permission.name == "generate_content")
            )
        ).scalar_one_or_none()
        if not perm:
            perm = Permission(
                name="generate_content", description="Can generate AI content"
            )
            session.add(perm)
            await session.commit()
    ...
    async with AsyncTestingSessionLocal() as session:
        result = await session.execute(
            select(User)
            .where(User.username == "alice_test")
    ...
    assert (
        data is not None
    ), f"Create article response is None! Status: {response.status_code}, Text: {response.text}"
    ...
    assert (
        data2 is not None
    ), f"Create 2nd article response is None! Status: {response2.status_code}, Text: {response2.text}"
    ...
    assert (
        tags is not None
    ), f"List tags response is None! Status: {tags_response.status_code}, Text: {tags_response.text}"
    ...
    ), f"List articles response is None! Status: {articles_response.status_code}, Text: {articles_response.text}"
    ...
    assert (
        updated is not None
    ), f"Update article response is None! Status: {update_response.status_code}, Text: {update_response.text}"
    ...
    assert (
        ai_data is not None
    ), f"AI generate response is None! Status: {ai_gen_response.status_code}, Text: {ai_gen_response.text}"
    ...
    assert (
        ai_sum_data is not None
    ), f"AI summarize response is None! Status: {ai_sum_response.status_code}, Text: {ai_sum_response.text}"
    ...
    assert (
        del_data is not None
    ), f"Delete article response is None! Status: {del_response.status_code}, Text: {del_response.text}"
