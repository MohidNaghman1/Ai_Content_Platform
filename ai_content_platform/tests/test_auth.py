...
async def test_register_login_refresh_rbac(client):
    ...
    response = await client.post("/auth/login", data={"username": "alice_test", "password": "string"})
    assert response.status_code == 200
    ...
    refresh_response = await client.post("/auth/token/refresh", json={"refresh_token": refresh_token})
    ...
    revoke_response = await client.post("/auth/token/revoke", json={"refresh_token": refresh_token})
    assert revoke_response.status_code == 200

    # Try to use revoked token
    refresh_response2 = await client.post("/auth/token/refresh", json={"refresh_token": refresh_token})
    assert refresh_response2.status_code in (401, 404)

    # Test login and access with viewer role
    response = await client.post("/auth/login", data={"username": "bob_test", "password": "string"})
    assert response.status_code == 200
    tokens = response.json()
    access_token = tokens["access_token"]
    headers = {"Authorization": f"Bearer {access_token}"}
